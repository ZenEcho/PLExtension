...
function PLNotification(config) {
    ...
    ${config.duration > 0 ? `<div class="progress-bar"></div>` : ''}
`;
...
function updateCountdown(config, notification) {
    let progressBar; // 用于存储进度条元素
    let timer; // 用于存储倒计时计时器
    progressBar = notification.querySelector('.progress-bar');
...
// 从浏览器的 storage 移除通知
function removeNotificationFromStorage(notification) {
    const storageAPI = typeof browser !== 'undefined' && browser.storage ? browser.storage : typeof chrome !== 'undefined' && chrome.storage ? chrome.storage : null;
...
window.addEventListener('message', function (event) {
    if (event.data.type === 'PLNotification') {
        PLNotification(event.data.data)
    }
});
